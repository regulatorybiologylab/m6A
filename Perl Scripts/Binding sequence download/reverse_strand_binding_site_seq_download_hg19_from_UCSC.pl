...
open(DATA3,">data_seq_reverse.txt");
while($sukhen=<DATA1>)
{
open(MYFILE, ">data_reverse.txt");
@a=split('\t',$sukhen);
$p=$a[0];
$q=$a[1];
$r=$a[2];
...
open(DATA1, "<data_seq_reverse.txt");
open(DATA2, ">data_seq_reverse_nr.txt");
while($a=<DATA1>)
   {
  if($a=~/>/)
     {
...
#!/usr/bin/perl
open(DATA2, ">sequences_of_reverse_stand.csv");
...
$com=$a[0].":".$a[1]."-".$a[2];
print DATA2 $a[4];
open(DATA1, "<data_seq_reverse_nr.txt");
 while($sukhen2=<DATA1>)
  {
